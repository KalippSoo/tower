...
public class PlayManager extends EventListener{
	...
	public PlayManager() {
		currentRoom.add(new Play(new Location(Utils.world, -47.5, 104, -3.5), 0, 2));
					...
					if (playRoom.getPlayers().size() > 0) {
						...
						if (playRoom.current == 0) {
							...
							playRoom.getPlayers().forEach(players->{
								callEvent(new PlayerTeleportPlayEvent(playRoom, ArenaManager.searchPlaceToPlay(playRoom, playRoom.getMode())));
							});
						}
					}
	...
	public void OnPlayerTeleportPlayEvent(PlayerTeleportPlayEvent e) {
		...
		for (Player player : players) {
			...
			data.play=null;
			data.setArena(e.getArena());
			player.teleport(e.getArena().tp);
